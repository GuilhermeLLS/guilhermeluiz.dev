let getStyles = (theme: string) =>
  switch theme {
  | "light" => "text-white dark:text-gray-700"
  | "dark" => "dark:text-white text-gray-700"
  | _ => ""
  }

module GitHubIcon = {
  @react.component
  let make = (~url: string, ~theme: string) => {
    <a className="mx-2" href={url} target="_blank" rel="noreferrer">
      <svg
        className={`w-5 ${getStyles(
            theme,
          )} h-5 fill-current hover:text-indigo-500 dark:hover:text-indigo-400`}
